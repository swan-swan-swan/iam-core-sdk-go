...
- [x] **Step 1: 修改 create/update security 期望请求体并加入旧响应兼容输入**
...
- [x] **Step 2: 删除 request body 字段与 recommended 校验**
...
- [x] **Step 3: 运行 management package tests 验证 GREEN**
...
- [x] **Step 1: 运行 S256 focused tests**
...
- [x] **Step 3: 执行 SDK 完整验证**

Run: `go test ./... -count=1 && go vet ./... && git diff --check`

Expected: 全部通过。

**Verification result (2026-08-18):** 隔离提交快照下 `go test ./... -count=1`、`go vet ./...` 与 `git diff --check` 通过；旧字段仅保留在私有 wire 解码层用于滚动升级。
